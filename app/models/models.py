from sqlalchemy import Column, Integer, String, Text, ForeignKey, JSON, Boolean, BigInteger, DateTime
...
class Client(BaseModel):
    ...
    client_id = Column(String(32), unique=True, nullable=False, comment='客户端唯一标识')
...
class Controller(BaseModel):
    """控制器表"""
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    controller_id = Column(String(32), unique=True, nullable=False, comment='控制器唯一标识')
    client_id = Column(String(32), ForeignKey('client.client_id'), nullable=False, comment='所属客户端ID')
...
class Message(BaseModel):
    """消息记录表"""
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    controller_id = Column(String(32), ForeignKey('controller.controller_id'), nullable=False, comment='控制器ID')
    message_type = Column(Integer, nullable=False, comment='消息类型：1-文本，2-图片，3-文件')
    ...
    chat_type = Column(Integer, nullable=False, comment='聊天类型：1-私聊，2-群聊，3-频道')
...
class ScheduledTask(BaseModel):
    ...
    controller_id = Column(String(32), ForeignKey('controller.controller_id'), nullable=False, comment='控制器ID')
    task_type = Column(Integer, nullable=False, comment='任务类型：1-消息推送，2-好友添加，3-频道检索')
